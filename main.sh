...
REPO="https://raw.githubusercontent.com/Hamadasiana/hamada1/main/"
...
if [ -f /root/domain ]; then
  ...
  echo "لم يتم العثور على ملف /root/domain."
  echo -e "اختر نوع الدومين:
1. دومين من Cloudflare
2. دومين عشوائي من السيرفر"
"
  read -p "اختر [1/2]: " option
  if [ "$option" == "1" ]; then
    read -p "ادخل دومين Cloudflare الخاص بك: " domain
  else
    domain=$(curl -s ipv4.icanhazip.com).nip.io
  fi
  echo "$domain" > /root/domain
fi
else
  echo "⚠️ الملف /root/domain غير موجود، يرجى إنشاؤه يدويًا ووضع اسم النطاق الخاص بك."
  exit 1
fi
else
...
"
  ...
  exit 1
    ...
    curl https://raw.githubusercontent.com/Hamadasiana/hamada1/main/acme.sh -o /root/.acme.sh/acme.sh
